...
- (void)viewDidLoad {
    ...
    [self.navigationController.navigationBar setTitleTextAttributes:
     @{NSForegroundColorAttributeName:[UIColor redColor],
       NSFontAttributeName:[UIFont fontWithName:@"Calibri-Bold" size:120]}];
...
- (void) viewWillAppear:(BOOL)animated
{
    ...
    NSString * device_token = [[NSUserDefaults standardUserDefaults] objectForKey:@"deviceid"];
    
    if (device_token != nil && device_token.length > 13) {
        [GlobalPool sharedObject].m_strDeviceToken = device_token;
        NSDictionary* dictLoginInfo = [[GlobalPool sharedObject] getLoginInfo];
        ...
        [self performSelector:@selector(checkAutoLogin) withObject:nil afterDelay:1.0f];
}
       ...
       parameters:params
          success:^(AFHTTPRequestOperation *operation, id responseObject) {
              ...
              if ([[responseObject valueForKey:@"success"] boolValue])
              {
                  ...
                  [[NSUserDefaults standardUserDefaults] setObject:@"AccessToken" forKey:[GlobalPool sharedObject].m_strAccessToken];
                  [[NSUserDefaults standardUserDefaults] setObject:@"username" forKey:[GlobalPool sharedObject].m_strCurUsername];
                  [[NSUserDefaults standardUserDefaults] setObject:@"id" forKey:[GlobalPool sharedObject].m_strCurUserID];
                  [[NSUserDefaults standardUserDefaults] setObject:@"latitude" forKey:[GlobalPool sharedObject].m_strLatitude];
                  [[NSUserDefaults standardUserDefaults] setObject:@"longitude" forKey:[GlobalPool sharedObject].m_strLongitude];
                  [[NSUserDefaults standardUserDefaults] setObject:@"city" forKey:[GlobalPool sharedObject].m_strCity];
...
- (IBAction)actionSignup:(id)sender {
    ...
    VerifyViewController* viewCon = [storyboard instantiateViewControllerWithIdentifier:@"inviteCode"];
