...
- (IBAction)actionRateUs:(id)sender {
    ...
    NSString* strRequestLink = [[GlobalPool sharedObject] makeAPIURLString:@"appid"];
    [manager POST: strRequestLink
       parameters:nil
          success:^(AFHTTPRequestOperation *operation, id responseObject) {
              NSLog(@"JSON: %@", responseObject);
              
              NSString * app_id = [responseObject valueForKey:@"app_id"];
              
              //ios7 before
              NSString *reviewURL = [templateReviewURL stringByReplacingOccurrencesOfString:@"APP_ID" withString:app_id];
