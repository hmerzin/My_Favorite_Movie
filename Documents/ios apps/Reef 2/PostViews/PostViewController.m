...
- (void)viewDidLoad {
    ...
    self.m_postText.autocorrectionType = UITextAutocorrectionTypeYes;
    self.m_postText.spellCheckingType = UITextSpellCheckingTypeYes;
}
...
//    }
    ...
    NSString * postedTitle = @"";
    if (![self.m_postText.text isEqualToString:POST_TEXT_PLACEHOLDER]) {
        postedTitle = self.m_postText.text;
    }
    
    NSData *data = [postedTitle dataUsingEncoding:NSNonLossyASCIIStringEncoding];
