       ...
       parameters:params
          success:^(AFHTTPRequestOperation *operation, id responseObject) {
              ...
              if ([[responseObject valueForKey:@"success"] boolValue])
              {
                  ...
                  m_arrResult = [self sortImages:m_arrResult];
                  
                  NSString * username = [GlobalPool sharedObject].m_strCurUsername;
                  
                  if (m_arrResult != nil || m_arrResult.count > 0) {
                      [GlobalPool sharedObject].isTop = [self isEqualUsername:username anotheruser:[[m_arrResult firstObject] objectForKey:@"username"]];
                      
                      [self sendPushMessage:[GlobalPool sharedObject].isTop];
                  }
                  
                  [GlobalPool sharedObject].isNothing = [self isNothing:m_arrResult username:username];
                  [self sendPushMessage:[GlobalPool sharedObject].isNothing];
...
- (void)sendPushMessage:(BOOL)isSend {
    if (isSend) {
        NSLog(@"send push");
        ...
        NSString* strRequestLink = [[GlobalPool sharedObject] makeAPIURLString:@"send_ios_push"];
        NSString * message = @"";
        
        if ([GlobalPool sharedObject].isTop) {
            message = @"You have top photo now";
        }
        else if ([GlobalPool sharedObject].isNothing)   {
            message = @"You posted nothing for 2 days";
        }
        
        NSDictionary* params = @{@"deviceToken":[GlobalPool sharedObject].m_strDeviceToken, @"message":message};
        [manager POST: strRequestLink
           parameters:params
              success:^(AFHTTPRequestOperation *operation, id responseObject) {
                  NSLog(@"JSON: %@", responseObject);
...
- (BOOL)isEqualUsername:(NSString *) username anotheruser:(NSString *)anotherUser  {
    
    if ([username isEqualToString:anotherUser]) {
        return true;
    }else   {
        return false;
    }
...
- (BOOL)isNothing:(NSMutableArray *)postedImages username:(NSString *)username    {
    
    if (postedImages == nil || postedImages.count == 0) {
        return YES;
    }
    else {
        BOOL isNothing;
        for(int i = 0; i < postedImages.count; i ++) {
            if ([self isEqualUsername:username anotheruser:[[postedImages objectAtIndex:i] objectForKey:@"username"]]) {
                isNothing = NO;
                break;
            }
        }
...
- (int)compareObject:(NSDictionary * )posteImage anotherImage:(NSDictionary *)anotherImage {
    
    float viewed_time = [(NSString *)[posteImage objectForKey:@"viewed_time"] floatValue];
    float viewed_user = [(NSString *)[posteImage objectForKey:@"viewed_user"] floatValue];
    
    float another_viewed_time = [(NSString *)[posteImage objectForKey:@"viewed_time"] floatValue];
    float another_viewed_user = [(NSString *)[posteImage objectForKey:@"viewed_user"] floatValue];
    
    if (viewed_time/viewed_user > another_viewed_time/another_viewed_user) {
        return -1;
    } else if (viewed_time/viewed_user == another_viewed_time/another_viewed_user)  {
        return 0;
    }
    else    {
        return 1;
...
- (NSMutableArray *)sortImages:(NSMutableArray *)sourceArray
{
    [sourceArray sortUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
       
        return [self compareObject:obj1 anotherImage:obj2];
