...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"Reef";
    
    [self.navigationController.navigationBar setTitleTextAttributes:
     @{NSForegroundColorAttributeName:[UIColor whiteColor],
       NSFontAttributeName:[UIFont fontWithName:@"Calibri-Bold" size:24]}];
    ...
    [manager POST: strRequestLink
      parameters:params
         success:^(AFHTTPRequestOperation *operation, id responseObject) {
             ...
             if (m_arrData != nil && [m_arrData count] == 0) {
                 m_arrData = [[NSMutableArray alloc] init];
             } else
                 [m_arrData  removeAllObjects];
             ...
             if ([[responseObject valueForKey:@"success"] boolValue])
             {
                 ...
                 if (m_arrData.count > 0)
                 {
                     ...
                     [GlobalPool sharedObject].posted_photos = [NSMutableArray arrayWithArray:m_arrData];
                 if ([GlobalPool sharedObject].posted_photos != nil && [GlobalPool sharedObject].posted_photos.count > 9) {
                     [self sendPushMessage:YES];
                 }
...
- (void)sendPushMessage:(BOOL)isSend {
    if (isSend) {
        ...
        NSDictionary* params = @{@"deviceToken":[GlobalPool sharedObject].m_strDeviceToken
                                 , @"message":@"You have over the 10 of photos at home"};
        [manager POST: strRequestLink
           parameters:params
              success:^(AFHTTPRequestOperation *operation, id responseObject) {
                  NSLog(@"JSON: %@", responseObject);
...
- (void)touchDoubleTapView:(FeedSubView *)subView   {
    ...
    if (nAutoScrollIndex == 0) {
        nAutoScrollIndex = m_arrResult.count - 1;
    } else {
        nAutoScrollIndex = nAutoScrollIndex -2;
    }
...
- (void) didDoubleTapOnZoomableViewForCell  {
